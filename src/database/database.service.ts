...
export class DatabaseService {
    ...
    createUser(body: {login: string, password: string} ): Omit<User, 'password'> {
      ...
      deleteArtist(id: string): any {
        ...
        const tracksId = localDB.tracks.filter((t)=> t.artistId === id).map((t)=> t.id);
        const albumsId = localDB.albums.filter((t)=> t.artistId === id).map((t)=> t.id);
        tracksId.forEach((t)=> {
      ...
      createTrack(body: {name: string, artistId: string, albumId: string, duration: number } ): Track {
        const track = {
          id: uuidv4(),
          name: body.name,
      ...
      updateTrack(id: string, body: {name?: string, artistId?: string, albumId?: string, duration?: number } ): Track {
      ...
      updateAlbum(id: string, body: {name?: string, artistId?: string, year?: number } ): Album {
      ...
      deleteAlbum(id: string): any {
        ...
        const tracksId = localDB.tracks.filter((t)=> t.albumId === id).map((t)=> t.id);
